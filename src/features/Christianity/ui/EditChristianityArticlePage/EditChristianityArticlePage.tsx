...
const EditChristianityArticlePage = () => {
    ...
    const { article, loading, categories, updateArticle} = useEditChristianityArticlePage();
    ...
    return (
        <Container size="md" py="md">
            <Button mt="md" mb="sm" variant="outline" component={Link} to={`/admin/christianity/articles`}>Повернутись до списку</Button>
            <Title mb="sm">Редагування статті</Title>
            ...
            <Button color="green" size="md" mb="md">Створити новий переклад</Button>
            <Grid>
                {!!article && article.translations.map((translation) => (
                    <Grid.Col span={{ base: 12, sm: 6, md: 6 }} key={translation.id}>
                        <Card key={translation.id} shadow="sm" padding="lg" radius="md" withBorder>
                            ...
                            <Button mt="xs" component={Link} to={`/admin/christianity/translations/${article.slug}/${translation.language}`}>
                                Переглянути
                            </Button>
                        </Card>
                    </Grid.Col>
