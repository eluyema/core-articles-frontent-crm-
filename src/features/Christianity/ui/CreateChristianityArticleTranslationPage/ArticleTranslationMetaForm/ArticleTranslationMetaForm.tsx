...
import {
    ...
    TextInput,
    Image,
    Textarea, Combobox, SelectProps, Select, Tooltip
} from "@mantine/core";
...
const ArticleTranslationMetaForm = ({handleSubmitMetaForm}: ArticleMetaFormProps) => {
    ...
    const { form, loading, onSubmit } = useArticleTranslationMetaForm(handleSubmitMetaForm);

    return <form onSubmit={onSubmit}>
        <Box mt="lg">
            <Group flex="row" align="center">
                <Tooltip multiline w={270} withArrow transitionProps={{ transition: 'fade-left', duration: 300 }}  label="Зверніть увагу що мова перекладу має бути унікальною для статті!" position="right">
                <Select
